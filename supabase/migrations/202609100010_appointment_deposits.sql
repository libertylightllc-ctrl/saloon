...
declare
  ...
  cash_deposits numeric := 0;
  cash_deposit_refunds numeric := 0;
...
begin
  ...
  select coalesce(sum(coalesce(
    nullif(record.data->>'cashAmount','')::numeric,
    case when record.data->>'payment'='Cash' then (record.data->>'amount')::numeric else 0 end
  )),0) into cash_sales
  ...
  from public.salon_records record where record.shop_id=target_shop and record.record_type='refund'
    and record.deleted_at is null
    and left(record.data->>'createdAt',10)=business_date::text;
  select coalesce(sum((record.data->>'deposit')::numeric),0) into cash_deposits
  from public.salon_records record where record.shop_id=target_shop and record.record_type='queue_ticket'
    and record.deleted_at is null and coalesce((record.data->>'deposit')::numeric,0)>0
    and coalesce(record.data->>'depositPayment','Cash')='Cash'
    and left(record.data->>'createdAt',10)=business_date::text;
  select coalesce(sum((record.data->>'deposit')::numeric),0) into cash_deposit_refunds
  from public.salon_records record where record.shop_id=target_shop and record.record_type='queue_ticket'
    and record.deleted_at is null and record.data->>'depositStatus'='Refunded'
    and coalesce(record.data->>'depositPayment','Cash')='Cash'
    and left(record.data->>'cancelledAt',10)=business_date::text;
  ...
  expected_amount := opening_amount + cash_sales + cash_deposits - cash_deposit_refunds - cash_refunds - cash_purchases - cash_expenses - cash_supplier_payments - cash_payroll;
  ...
  authoritative := closing_data || jsonb_build_object(
    ...
    'openingCash',opening_amount,'cashSales',cash_sales,'cashDeposits',cash_deposits,
    'cashDepositRefunds',cash_deposit_refunds,'cashRefunds',cash_refunds,
